...
var debug = (0, _debug2.default)('appifi:appstore');
var K = function K(x) {
  return function (y) {
    return x;
  };
};
var useLocalRecipes = false; // used for test, obsolete
...
var getJsonRecipesUrl = function getJsonRecipesUrl() {
  ...
  var url = (0, _reducers.storeState)().developer && (0, _reducers.storeState)().developer.appstoreMaster === true ? 'https://raw.githubusercontent.com/wisnuc/appifi-recipes/master/release.json' : 'https://raw.githubusercontent.com/wisnuc/appifi-recipes/release/release.json';

  debug('using ' + url);
  return url;
};

var retrieveTextAsync = (0, _bluebird.promisify)(function (url, callback) {
  return _superagent2.default.get(url).set('Accept', 'text/plain').end(function (err, res) {
    if (err) callback(err);else if (!res.ok) callback(new Error('bad response'));else callback(null, res.text);
  });
});
